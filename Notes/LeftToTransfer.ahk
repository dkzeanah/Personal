...
MinMax(title)
{
   if WinExist(title)
   {
      ((WinActive(title)) ? (WinMinimize()) : (WinActivate()))
      return 1
   }
   else
      return 0
...
;Run an app using its exe, activate the app's window
RunApp(appDirectory)
{
...
;Run app, but only if the window doesn't exist already. Takes the ahk_exe instead of pure directory as parameter, trims it to the pure directory later. Not the initial version because it would be excessive in use with MinMax. 
IfNotExist_RunApp(ahk_exe_byFullDirectory)
{
   if !WinExist(ahk_exe_byFullDirectory)
   {
      ahk_exe_appDirectory_Regex_slashes := StrReplace(ahk_exe_byFullDirectory, "\\", "\")
      ahk_exe_appDirectory_Regex_finished := StrReplace(ahk_exe_appDirectory_Regex_slashes, "\.", ".")
      fullDirectory := LTrim(ahk_exe_appDirectory_Regex_finished, "ahk_exe ")
      Run, %fullDirectory%,, Max 
      WinWait, %ahk_exe_byFullDirectory%
   } 
}
...
{
   appDirectory_RegEx_slashes := StrReplace(appDirectory, "\", "\\")
   appDirectory_RegEx_finished := StrReplace(appDirectory_RegEx_slashes, ".", "\.")
   ((!MinMax("ahk_exe " appDirectory_RegEx_finished)) ? (RunApp(appDirectory)))
}
...
;Winapp, but for folders. SetTitleMode, RegEx checks the win title by "consists" rather than full precise title. \ and . are special chars in RegEx, so backslashes are added to escape them
WindowsApp_Folders(appDirectory)
{
   ...
   Switch
   {
      Case !WinExist(appDirectory_RegEx_finished):RunWait, %appDirectory%
      Case WinActive(appDirectory_RegEx_finished):WinMinimize 
      Case !WinActive(appDirectory_RegEx_finished):WinActivate
   }
}
